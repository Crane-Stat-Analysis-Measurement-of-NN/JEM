...
def checkpoint(f, opt, buffer, epoch_no, tag, args, device):
    ...
    print(os.path.join(args.save_dir,tag)," checkpoint saved.")
...
def main(args):
    ...
    set_up_experiment(args, args.seed)
    
    # datasets
    dload_train, dload_train_labeled, dload_valid, dload_test, dtrain_size = get_data(args)
    ...
    with open(os.path.join(args.save_dir,'epoch_times.csv'),'a') as timef:
        print(f'start,{time.time()}\n')
        timef.write(f'start,{time.time()}\n')
...
# "save_dir": './production/ours_nb/run5'
# "p_x_weight": 0 (SGLD)
# "p_y_given_x_weight": 1.0 (X-ent)
# "p_x_y_weight": 0.0 (???)
# "new_energy": 1.0
